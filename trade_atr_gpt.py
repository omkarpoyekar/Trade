...
def post_atr(loop_sleep=60, target_equity_return_pct=0.015):
    ...
    with TRAIL_LOCK:
        ...
        if not t:
            ...
            entry = float(pos["entry_price"])
            stop = entry - K_ATR * atr if pos["side"] == "buy" else entry + K_ATR * atr
            t = {"side": pos["side"], "watermark": entry, "stop": stop}
        else:
            watermark = float(t["watermark"])
            prev_stop = t.get("stop")  # might be None
            if prev_stop is None:
                # First ATR: set an initial stop from watermark
                stop = (watermark - K_ATR * atr) if pos["side"] == "buy" else (watermark + K_ATR * atr)
            else:
                prev_stop = float(prev_stop)
                if pos["side"] == "buy":
                    stop = max(prev_stop, watermark - K_ATR * atr)
                else:
                    stop = min(prev_stop, watermark + K_ATR * atr)
            t["stop"] = stop
    ...
    return jsonify({"ok": True, "message": "Trailing stop updated.", "stop": t["stop"], "watermark": t["watermark"]}), 200
            ...
            if pos:
                ...
                if t:
                    ...
                    if px is not None:
                        ...
                        elif pos["side"] == "sell" and px >= stop:
                            prev = pos.copy()
                            pnl = close_position(state, px)
                            log_trade(
                                action="close", side=prev["side"], price=px, qty_btc=prev["qty_btc"],
